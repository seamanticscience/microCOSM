...
def run_boxmodel(df_input,df_parms,df_aux,df_refs,workers=1,Rcp=106,Rnp=16):
    ...
    df_tim["nyrs"].where(np.logical_and((df_parms["lambda"]/3e7)>(10**2),
                                        (df_parms["gamma"]/Rcp)<(10**-6)),1.e4,inplace=True)
    df_tim["nyrs"].mask (np.logical_and((df_parms["lambda"]/3e7)>=(10**2),
                                        (df_parms["gamma"]/Rcp)<=(10**-6)),1.e5,inplace=True)
    df_tim["tout"].where(np.logical_and((df_parms["lambda"]/3e7)>(10**2),
                                        (df_parms["gamma"]/Rcp)<(10**-6)),10. ,inplace=True)
    df_tim["tout"].mask (np.logical_and((df_parms["lambda"]/3e7)>=(10**2),
                                        (df_parms["gamma"]/Rcp)<=(10**-6)),100.,inplace=True)
